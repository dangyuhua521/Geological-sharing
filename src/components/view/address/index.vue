        ...
        <div :class="$style.tableBox">
          <div :class="$style.searchs">
            ...
            >
              ...
              <!-- <el-button type="buttoms" @click="searchDepartment2()" size="small" style="margin-left:11px; width:60px;">
              <i class="el-icon-search"></i></el-button>-->
            </el-form>
          </div>
          <p :class="$style.searchTotal">共找到{{ parseInt(total) || 0 }}个</p>
          <el-table
            ...
            tooltip-effect="dark"
          >
            <el-table-column type="index" label="序号" show-overflow-tooltip width="50px"></el-table-column>
            <el-table-column prop="title" label="标题" show-overflow-tooltip width="550px">
              <template slot-scope="scope">
                <a style="color: #409eff;" @click="details(scope.row)">{{ scope.row.title }}</a>
              </template>
            </el-table-column>
            <!-- <el-table-column prop="pubunit" label="发布单位" show-overflow-tooltip></el-table-column> -->
            <el-table-column prop="pubdate" label="发布时间" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>
                  {{
...
import config from '../../view/userManager/orderManagement/config'
...
export default {
  ...
  methods: {
    ...
    clickDownload() {
      ...
      $eleForm.attr(
        'action',
        this.api + '/TYwTechnicalnorm/DownloadFiles?id=' + id
      )
